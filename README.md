...
*   📚 **知识库管理 (RAG)**：
    *   **混合存储**：结合 SQLite (元数据) 与 Qdrant (向量) 确保数据一致性。
    *   **异步处理**：基于 FastAPI BackgroundTasks 的异步索引机制，大文件上传不阻塞。
    *   **目录同步**：一键扫描 `knowledge_base/` 目录，自动化完成解析、切分与向量化。
*   ⚖️ **审核标准设置**：支持“严格合规”、“标准研判”、“建设性指导”三种 AI 审核模式切换。
*   📊 **文档深度分析**：支持上传 **PDF/Excel/Word**，自动提取财务指标与 ESG 风险。
*   🛠️ **扩展插件系统**：支持 **MCP (Model Context Protocol)** 与自定义 API 工具集成。
*   ⚡ **流式响应 (SSE)**：全链路流式输出，内置思维链 (CoT) 展示。
*   🎨 **现代 UI**：复刻 Gemini 风格界面，支持文档预览、历史管理与响应式设计。
